define(['user/module', 'user/service'], function(userModule) {
	return userModule
		.controller('UserDetail', ['$scope', '$http', '$state', 'userService', function($scope, $http, $state, userService) {
			var self = this;
			self.form = {}; // 要提交的表单数据
			...
			self.modal = {
				...
				whenConfirm: function() {
					if(self.detail.empNum && self.detail.empNum > 0) {
						userService.updateEmployee(self.form).then(function(resp) {
							// $state.go('user.detail', { id : self.form.id }, { reload : true });
							self.getDetail(self.form.id);
						});
					} else {
						userService.updateCustomer(self.form).then(function(resp) {
							// $state.go('user.detail', { id : self.form.id }, { reload : true });
							self.getDetail(self.form.id);
						});
					}
			...
			self.enableUser = function() {
				...
				userService.enableUser(id).then(function(resp) {
					self.getDetail(id);
				});
			};
			self.disableUser = function() {
				...
				userService.disableUser(id).then(function(resp) {
					self.getDetail(id);
				});
			};
