...
import com.github.emailtohl.integration.flow.service.ApplicationFormService;
import com.github.emailtohl.integration.web.dto.ApplicationFormDto;
...
public class ApplicationFormCtrl {
	...
	@ResponseBody
	public Paging<ApplicationForm> findMyApplicationForm(@PageableDefault(page = 0, size = 10, sort = BaseEntity.CREATE_DATE_PROPERTY_NAME, direction = Direction.DESC) Pageable pageable) {
		Page<ApplicationForm> page = applicationFormService.findApplicationFormByEmail(CtrlUtil.getCurrentUsername(), pageable);
		return new Paging<ApplicationForm>(page.getContent(), page.getTotalElements(), pageable.getPageNumber(), pageable.getPageSize());
	}
	...
	@ResponseBody
	public Paging<ApplicationForm> queryApplicationForm(@RequestParam(name = "name", required = false) String name, 
	...
	@ResponseBody
	public Paging<ApplicationHandleHistory> history(
		...
		return new Paging<ApplicationHandleHistory>(page.getContent(), page.getTotalElements(), pageable.getPageNumber(), pageable.getPageSize());
	}
